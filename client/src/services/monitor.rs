...
impl FileMonitor {
    ...
    pub async fn start<F, Fut>(&self, file_path: &str, on_update: F) where
        ...
        Fut: Future<Output = Result<(), AppError>> + Send + 'static,
    {
        ...
        TaskManager::instance().add(&format!("FileMonitor-{}", file_path), monitor).await;
    }

    pub async fn stop(&self, file_path: &str) {
        TaskManager::instance().dispose(&format!("FileMonitor-{}", file_path)).await;
    }

    async fn start_monitor<F, Fut>(file_path: &str, on_update: F) -> Result<(), AppError> where
        F: Fn(String) -> Fut + Send + Sync + 'static,
        Fut: Future<Output = Result<(), AppError>> + Send + 'static,
    {
        ...
        loop {
            ...
            while let Ok(bytes_read) = reader.read_line(&mut line).await {
                ...
                if !trimmed_line.is_empty() {
                    let _ = on_update(trimmed_line.to_string()).await;
                }
...
impl InstructionMonitor {
    pub async fn start<F, Fut>(on_update: F) where
        F: Fn(String) -> Fut + Send + Sync + 'static,
        Fut: Future<Output = Result<(), AppError>> + Send + 'static,
    {
        FileMonitor::instance().start(INSTRUCTION_FILE_PATH, on_update).await;
    }
